...
6. **Structural proof**: extract IR of the new version, `marinade diff`
   vs prior, classify every change into expected buckets; anything
   outside the list is a finding.
7. Update the change docs / version log.

## Find the seam before you cut

- **Consumer census** (SKILL.md has the UNION query): cross-sheet edge
  counts identify the interface sheet whose output layout you must
  preserve; everything behind it is replaceable. A large blast radius
  with a boundary is tractable surgery; without the boundary it isn't.
- **Anchor discovery**: grep `formulas.formula_r1c1` for defined names
  before assuming a wide edit — one anchor name can mean repointing two
  cells and touching nothing downstream.
- **Dead-tab check**: zero consumers → safe to delete; nonzero → escalate
  as a decision, don't decide unilaterally.
- **Comment inventory**: `cell_comments` before/after a rebuild — the
  owner's cell comments must survive.
...
1. Enumerate the changed cells' direct consumers from the IR edges.
...
4. Prove NON-paths: "sheet B has zero references to sheet A" is an IR
...
- Extract every version once (background; cache `.ir.db`s in scratch),
  then `marinade diff` on each consecutive pair, summarizing counts per
  change type first; drill into the full changelist only for pairs whose
  summary is unexpected. Review every
  formula/value change against documented intent — anything not traceable
  to a documented change is flagged.
- A zero-unexplained-changes claim covers the **workbook edits** section
  only. The diff's "IR INFERENCE" section (table candidates, label
  evidence, time annotations; `layer='ir_inference'` in JSON) is
  extractor inference that churns benignly across versions — it needs no
  per-event adjudication and must not be counted as edits.
...
checklists are useful as method; Marinade supplies the two capabilities they
lack: dependency mapping and version diffing.
